 ...
 */
export const onWaterMetricsUpdate = onDocumentUpdated('system_settings/water_metrics', async (event) => {
 ...
 */
export const validateWaterMetrics = onDocumentWritten('system_settings/water_metrics', async (event) => {
    const change = event.data;
    if (!change || !change.after.exists) return; // Deletion or no data
    ...
    // Validation logic
    const errors: string[] = [];
    if (data.dailyPerPersonLiters <= 0) errors.push('Daily liters must be positive');
    if (data.dailyPerPersonLiters > 1000) errors.push('Daily liters seems excessively high');
...
const getApiKeyFromReq = (req: express.Request) => {
  return (req.headers['x-api-key'] as string) || DISCHUB_API_KEY;
};
